...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      |Shift(delta_x,delta_y,e)  => case eval_prog(e,env) of
			              NoPoints => NoPoints
				     |Point (x,y)  => Point (x + delta_x,y + delta_y)
				     |Line(m,b) => Line(m, b + delta_y - m * delta_x)
				     |VerticalLine x  => VerticalLine (x + delta_x)
				     |LineSegment (x1,y1,x2,y2) => LineSegment (x1+delta_x , y1+delta_y, x2+delta_x , y2+delta_y)
				     |_  => eval_prog (e,env)
